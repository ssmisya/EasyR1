...
if [ -z "$TMUX" ]; then
  # 创建一个新的tmux会话并执行此脚本
  ###
  SESSION_NAME="sft_refine_7b_step100_$(date +%Y%m%d_%H%M%S)"
...
###
export NO_PROXY="localhost,127.0.0.1,10.140.37.132,10.132.0.0/16,10.1.0.0/16"
export no_proxy="localhost,127.0.0.1,10.140.37.132,10.132.0.0/16,10.1.0.0/16"
...
mkdir -p $log_dir
log_file="${log_dir}/sft_refine_7b_step100_$(date +%Y%m%d_%H%M%S).log"
...
###
export RAY_ADDRESS="http://10.140.37.132:8212"

###
quotatype="reserved"
### sft是sft_7b.yaml
config_file="examples/configs/sft_7b_100.yaml"
###
...
###
checkpoint_dir="/mnt/petrelfs/sunhaoyu/visual-code/EasyR1/checkpoints/checkpoints_sft7b_100_refine"
...
###
python_command="python -m verl.trainer.main config=${config_file} worker.actor.model.model_path=${MODEL_PATH} ray_address=\"10.140.37.132:6312\""
