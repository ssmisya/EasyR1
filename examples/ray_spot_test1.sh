...
if [ -z "$TMUX" ]; then
  # 创建一个新的tmux会话并执行此脚本
  ###
  SESSION_NAME="ray_spot_test1_$(date +%Y%m%d_%H%M%S)"
...
###
export NO_PROXY="localhost,127.0.0.1,10.140.37.44,10.140.0.0/16,10.1.0.0/16"
export no_proxy="localhost,127.0.0.1,10.140.37.44,10.140.0.0/16,10.1.0.0/16"
...
mkdir -p $log_dir
log_file="${log_dir}/ray_spot_test1_$(date +%Y%m%d_%H%M%S).log"
...
# 确保你的 Ray 集群已在 10.140.37.44:6312 运行，Dashboard 在 10.140.37.44:8212
# Ray Job CLI 连接的是 Dashboard 地址
###
export RAY_ADDRESS="http://10.140.37.44:8212"

###
quotatype="spot"
config_file="examples/configs/ray_spot_test1.yaml"
###
# MODEL_PATH=/mnt/petrelfs/sunhaoyu/visual-code/llm_weights/Qwen2.5-VL-3B-Instruct
MODEL_PATH=/mnt/petrelfs/share_data/songmingyang/runs/tool_factory/sft/v1/Qwen2.5-VL-7B-Instruct-ToolSFTv1/checkpoint-100
###
checkpoint_dir="/mnt/petrelfs/sunhaoyu/visual-code/EasyR1/checkpoints/checkpoints_ray_test1"
...
###
python_command="python -m verl.trainer.main config=${config_file} worker.actor.model.model_path=${MODEL_PATH} ray_address=\"10.140.37.44:6312\""
