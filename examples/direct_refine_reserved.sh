...
if [ -z "$TMUX" ]; then
  # 创建一个新的tmux会话并执行此脚本
  ###
  SESSION_NAME="direct_refine_$(date +%Y%m%d_%H%M%S)"
...
mkdir -p $log_dir
log_file="${log_dir}/direct_refine_$(date +%Y%m%d_%H%M%S).log"
...
quotatype="reserved"
### direct是reserved1
config_file="examples/configs/tool_reserved1.yaml"
...
###
checkpoint_dir="/mnt/petrelfs/sunhaoyu/visual-code/EasyR1/checkpoints/checkpoints_direct_refine"
