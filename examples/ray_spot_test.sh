...
if [ -z "$TMUX" ]; then
  # 创建一个新的tmux会话并执行此脚本
  ###
  SESSION_NAME="ray_spot_test_$(date +%Y%m%d_%H%M%S)"
...
###
export NO_PROXY="localhost,127.0.0.1,10.140.37.132,10.140.0.0/16,10.1.0.0/16"
export no_proxy="localhost,127.0.0.1,10.140.37.132,10.140.0.0/16,10.1.0.0/16"
...
mkdir -p $log_dir
log_file="${log_dir}/ray_spot_test_$(date +%Y%m%d_%H%M%S).log"
...
###
quotatype="spot"
config_file="examples/configs/ray_spot_test.yaml"
...
checkpoint_dir="/mnt/petrelfs/sunhaoyu/visual-code/EasyR1/checkpoints/checkpoints_ray_test"
