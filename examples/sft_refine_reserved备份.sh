...
if [ -z "$TMUX" ]; then
  # 创建一个新的tmux会话并执行此脚本
  ###
  SESSION_NAME="sft_refine_$(date +%Y%m%d_%H%M%S)"
...
###
export NO_PROXY="localhost,127.0.0.1,10.140.37.138,10.140.0.0/16,10.1.0.0/16"
export no_proxy="localhost,127.0.0.1,10.140.37.138,10.140.0.0/16,10.1.0.0/16"
...
mkdir -p $log_dir
log_file="${log_dir}/sft_refine_$(date +%Y%m%d_%H%M%S).log"
...
###
export RAY_ADDRESS="http://10.140.37.138:8212"

###
quotatype="reserved"
config_file="examples/configs/tool_reserved.yaml"
# MODEL_PATH=/mnt/petrelfs/sunhaoyu/visual-code/llm_weights/Qwen2.5-VL-3B-Instruct
MODEL_PATH=/mnt/petrelfs/share_data/songmingyang/runs/tool_factory/sft/v1/Qwen2.5-VL-3B-Instruct-ToolSFTv1/checkpoint-245
checkpoint_dir="/mnt/petrelfs/sunhaoyu/visual-code/EasyR1/checkpoints/checkpoints_sft_refine"
...
###
python_command="python -m verl.trainer.main config=${config_file} worker.actor.model.model_path=${MODEL_PATH} ray_address=\"10.140.37.138:6312\""
...
ray job submit --address="$RAY_ADDRESS" \
  ...
  ray_address="10.140.37.138:6312" \
  $( [ -n "$latest_checkpoint" ] && echo "trainer.load_checkpoint_path=${latest_checkpoint}" ) \
  2>&1 | tee -a ${log_file}
